...
const { isFalse } = require("../../common/utils/functions");
...
class OptionService {
    ...
    async create(optionDto) {
        ...
        optionDto.key = slugify(optionDto.key, {trim: true, replacement:"_", lower: true})   //ddvd ijgl => ddvd_ijgl (not ddvd-ijgl)
    ...
    async update(id, optionDto) {
        ...
        async findByCategoryId(category) {
            return await this.#model.find({category}, {__v: 0}).populate({path: "category", select: {name: 1, slug: 1}}) //{__v: 0} doesn't work!!
        }
